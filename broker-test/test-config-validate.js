...
section('validateBrokerConfig: top-level');
{
  ...
  ok(
    'error mentions "must be an object"',
    r.errors.some((e) => /object/i.test(e.message)),
  );
}
{
...
section('clients validation');
{
  ...
  ok(
    'no "no admin" warning (admin present)',
...
}
{
  ...
  ok(
    'missing role → error',
    r.errors.some((e) => /role is required/.test(e.message)),
  );
}
{
  const r = validateBrokerConfig({
  ...
  ok(
    'unknown role → warning (forward-compat)',
    r.warnings.some((w) => /unrecognized role/.test(w.message)),
  );
}
{
  const r = validateBrokerConfig({
  ...
  ok(
    'allow_password_login=true without password → warning',
...
}
{
  ...
  ok(
    'unusual rate_limit format → warning',
...
}
{
  const r = validateBrokerConfig({
    clients: {
      ...
      client_rl_second: { role: 'developer', rate_limit: '5/second' },
      client_rl_unlimited: { role: 'readonly', rate_limit: 'unlimited' },
    },
  });
  ok('valid rate_limit values → no warning', !r.warnings.some((w) => /rate_limit/.test(w.message)));
}
{
  const r = validateBrokerConfig({
...
section('services validation');
{
  const r = validateBrokerConfig({
    ...
    services: {
      github: { upstream: 'https://api.github.com' },
      aliyun: { base_url: 'https://ecs.aliyuncs.com' },
...
}
{
  ...
  ok(
    ...
    r.errors.some((e) => /Invalid upstream URL|invalid URL/i.test(e.message)),
  );
}
{
  const r = validateBrokerConfig({
    clients: { c: { role: 'admin' } },
    services: { bad: { upstream: 'http://api.example.com' } },
  });
  ok(
    'public plain HTTP upstream → error',
    r.errors.some((e) => e.path === 'services.bad.upstream' && /Plain HTTP/.test(e.message)),
  );
}
{
  const r = validateBrokerConfig({
    clients: { c: { role: 'admin' } },
    services: { explicit: { upstream: 'http://api.example.com', allow_insecure_http: true } },
  });
  ok('explicit insecure HTTP override remains startable', r.errors.length === 0);
  ok(
    'explicit insecure HTTP override emits warning',
    r.warnings.some((w) => /unencrypted network/.test(w.message)),
  );
}
{
  const r = validateBrokerConfig({
    clients: { c: { role: 'admin' } },
    services: { bad: { upstream: 'https://user:pass@api.example.com' } },
  });
  ok(
    'embedded upstream credentials → error',
    r.errors.some((e) => /embedded credentials/.test(e.message)),
  );
}
{
  const r = validateBrokerConfig({
...
section('no admin warning');
{
  ...
  ok(
    'non-empty clients without admin → warning',
    r.warnings.some((w) => /no client with role admin/.test(w.message)),
  );
}
{
  ...
  ok(
    'empty clients → no "no admin" warning',
    !r.warnings.some((w) => /no client with role admin/.test(w.message)),
  );
}
...
section('preflightPaths');
{
  const r = preflightPaths(
    { configPath: fileURLToPath(new URL('../broker/package.json', import.meta.url)) },
    { existsSync },
  );
...
}
{
  ...
  ok('default ESM filesystem fallback works without injected fs', r.ok === true);
}
{
  const r = preflightPaths({ configPath: 'E:/this/does/not/exist.json' }, { existsSync });
  ok(
    'non-existing path → ok=false',
