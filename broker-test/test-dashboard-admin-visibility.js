...
console.log('=== dashboard scripts subscribe instead of 30s poll ===');
{
  ...
  const safePreview = readFileSync(join(root, '..', 'lib', 'safe-preview.js'), 'utf8');
  ...
  assert(!server.includes('error: e.message'), 'management failures do not audit raw exception text');
  ...
  assert(safePreview.includes('redactDeep(parsed)'), 'JSON service previews redact sensitive fields');
