...
function bootstrapPki(workDir) {
  ...
  // Compute fingerprint
  const fpOut = execFileSync(OPENSSL, [
...
function startMockUpstream(port) {
  ...
  return import('node:http').then(http => new Promise((resolve) => {
...
async function main() {
  ...
  try {
    ...
    broker.stdout.on('data', (d) => { brokerOut += d.toString(); });
    broker.on('exit', (code) => { exitCode = code; });
    ...
    ok('ws-stats has subscriber_count field',
       typeof wsStats.body.subscriber_count === 'number');
    ...
    const resolve = await mTLSPost('127.0.0.1', 18444, '/api/v1/secrets/resolve',
      { name: 'github_e2e_token' }, pki);
    ok('resolve via mTLS works', resolve.status === 200);
    // Response shape may vary; check any non-empty value field or fallback
    ok('resolve returns value (E2E-TOKEN-XYZ or wrapped)',
    ...
    ok('list contains github_e2e_token',
       JSON.stringify(list.body).includes('github'));

    // Proxy via mTLS — POST /api/v1/proxy/:service with {method, path} body
    const proxy = await mTLSPost('127.0.0.1', 18444, '/api/v1/proxy/github_e2e',
      { method: 'GET', path: '/user' }, pki);
    ok('proxy reaches upstream (200 expected)', proxy.status === 200);
    ok('proxy body matches mock',
       proxy.status === 200 && proxy.body && proxy.body.login === 'e2e-user');

    // Audit file should have entries
    const auditFile = join(workDir, 'audit', `audit-${new Date().toISOString().slice(0, 10)}.jsonl`);
