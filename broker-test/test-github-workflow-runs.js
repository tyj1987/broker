...
for (const excluded of ['unit-workflow-token', 'must not be projected', 'hidden/jobs', 'hidden/logs', 'pull_requests']) {
...
for (const [field, values] of Object.entries({
  ...
  for (const value of values) {
    await assert.rejects(adapter({ ...parameters, [field]: value }, context), expectCode('github_invalid_filter'));
  }
}
...
for (const execution of [
  ...
  await assert.rejects(adapter(parameters, { ...context, execution }), expectCode('github_execution_binding_mismatch'));
}
await assert.rejects(adapter(parameters, { ...context, accountRef: '' }), expectCode('github_account_unavailable'));

const withLease = (lease) => createGitHubWorkflowRunsListAdapter({
  now: () => NOW,
...
for (const [lease, code] of [
  ...
  [{ ...validLease(), permissions: { actions: 'read', contents: 'read' } }, 'github_credential_scope_mismatch'],
...
await assert.rejects(createGitHubWorkflowRunsListAdapter({
  now: () => NOW,
  tokenProvider: async () => { throw new Error('canary-token-detail'); },
  request: async () => ({ status: 200, body: validBody() }),
})(parameters, context), (error) => expectCode('github_credential_unavailable')(error) && !error.message.includes('canary'));

const withResponse = (response) => createGitHubWorkflowRunsListAdapter({
  now: () => NOW,
...
for (const [status, code] of [[401, 'github_credential_rejected'], [403, 'github_forbidden'],
  [404, 'github_not_found'], [400, 'github_filter_rejected'], [422, 'github_filter_rejected'],
  [429, 'github_upstream_error'], [500, 'github_upstream_error']]) {
  await assert.rejects(withResponse({ status })(parameters, context), expectCode(code));
}
for (const body of ['{bad-json', 'x'.repeat(1024 * 1024 + 1), {}, { total_count: -1, workflow_runs: [] },
  ...
  await assert.rejects(withResponse({ status: 200, body })({ ...parameters, per_page: 1 }, context),
    expectCode(typeof body === 'string' && body.length > 1024 * 1024 ? 'github_response_too_large' : 'github_invalid_response'));
}
await assert.rejects(createGitHubWorkflowRunsListAdapter({
  now: () => NOW,
  tokenProvider: async () => validLease(),
  request: async () => { throw new Error('canary-network'); },
})(parameters, context), (error) => expectCode('github_unavailable')(error) && !error.message.includes('canary'));
await assert.rejects(createGitHubWorkflowRunsListAdapter({
  now: () => NOW,
  tokenProvider: async () => validLease(),
  request: async () => { throw new V2Error('transport_policy_denied', 'safe', 403); },
...
const executor = createGitHubWorkflowRunsListExecutor({
  ...
  requestImpl,
  accountResolver: async ({ account_ref, environment }) => ({ account_ref, environment,
    client_id: 'Iv1.integration-test', installation_id: 12345, repositories: ['tyj1987/broker'] }),
...
assert.deepEqual(JSON.parse(calls[0].body), { repositories: ['broker'], permissions: { actions: 'read' } });
...
const taskBroker = new AutomationTaskBroker({
  ...
  authorize: async (operation) => ({
    allow: operation.provider === 'github' && operation.operationId === 'workflow_runs.list'
      && operation.accountRef === 'github-primary' && operation.environment === 'production'
...
const actor = { name: 'ci-observer', context: { via: 'workload_identity',
  client: { role: 'developer', principal_type: 'workload', security_profile: 'strict' } } };
const task = await taskBroker.create(actor, { tool: 'github.workflow-runs.list', tool_version: '1.0.0',
  account_ref: 'github-primary', environment: 'production', idempotency_key: 'github-runs-list-0001', parameters });
assert.equal(task.state, 'READY');
const completed = await taskBroker.run(actor, task.id);
assert.equal(completed.state, 'SUCCEEDED');
assert.deepEqual(taskBroker.eventsFor(actor, task.id).map((event) => event.state),
  ['REQUESTED', 'READY', 'EXECUTING', 'SUCCEEDED']);
assert.ok(taskEvents.every((event) => !JSON.stringify(event).includes('short-lived-workflow-token')));
await assert.rejects(taskBroker.run(actor, task.id), expectCode('invalid_state'));
assert.equal(calls.length, 4);
await assert.rejects(taskBroker.create(actor, { tool: 'github.workflow-runs.list', tool_version: '1.0.0',
  account_ref: 'github-primary', environment: 'production', idempotency_key: 'github-runs-injection-0001',
  parameters: { ...parameters, url: 'https://example.invalid/steal' } }), expectCode('schema_mismatch'));
assert.equal(calls.length, 4);

console.log('github workflow runs adapter: actions-read lease, bounded status and full task loop passed');
