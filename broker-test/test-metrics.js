...
import {
  inc, getCounter, observeMs, snapshot, prometheusText,
  _resetMetricsForTests, timedRequest,
...
section('inc / getCounter');
{
  ...
  ok('different labels are separate counters', getCounter('labeled{route="POST /api/v1/secrets"}') === 1);
  ...
  ok('label with quote and backslash does not throw', getCounter('tricky{msg="has \\"quote\\" and \\\\back"}') === 1);
}
...
section('observeMs');
{
  ...
  ok('bucket le=5 captures ms<=5 (cumulative)', s.histograms.lat.buckets[0].le === 5 && s.histograms.lat.buckets[0].count === 1);
  ok('bucket le=100 captures ms<=100 (cumulative)', s.histograms.lat.buckets[4].le === 100 && s.histograms.lat.buckets[4].count === 3);
...
section('prometheusText');
{
  ...
  ok('contains histogram bucket le=10', text.includes('http_duration_ms_bucket{service="broker",le="10"}'));
  ok('contains histogram +Inf bucket', text.includes('http_duration_ms_bucket{service="broker",le="+Inf"}'));
  ...
  ok('no labels → plain name', text2.includes('# TYPE http_requests counter') && text2.includes('http_requests 3'));
}
...
section('timedRequest');
{
  ...
  ok('timedRequest records duration', getCounter('broker_http_requests_total{route="GET /api/v1/me"}') === 1);
  const s = snapshot();
  ok('timedRequest observes latency histogram', s.histograms.broker_http_request_duration_ms.count === 1);

  // Error path still records
  let threw = false;
  try {
    await timedRequest('BAD', async () => { throw new Error('boom'); });
  } catch { threw = true; }
  ok('timedRequest propagates error', threw);
  ok('timedRequest still records on error', getCounter('broker_http_requests_total{route="BAD"}') === 1);
}
