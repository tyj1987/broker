...
install -o root -g root -m 0500 /bin/true "$TEST_ROOT/releases/release/bin/audit-signer"
...
for directory in "$TEST_ROOT" "$TEST_ROOT/releases" "$TEST_ROOT/releases/release" "$TEST_ROOT/releases/release/bin"; do
  setfacl -m "u:$GITHUB_USER:--x,u:$ALIYUN_USER:--x,u:$AUDIT_USER:--x,m::r-x" "$directory"
done
...
setfacl -m "u:$AUDIT_USER:r-x,m::r-x" "$TEST_ROOT/releases/release/bin/audit-signer"
...
runuser -u "$AUDIT_USER" -- "$TEST_ROOT/releases/release/bin/audit-signer"
...
deny runuser -u "$GITHUB_USER" -- test -r "$TEST_ROOT/releases/release/bin/audit-signer"
deny runuser -u "$ALIYUN_USER" -- test -r "$TEST_ROOT/releases/release/bin/audit-signer"
deny runuser -u "$AUDIT_USER" -- test -r "$TEST_ROOT/releases/release/bin/github-signer"
deny runuser -u "$AUDIT_USER" -- test -r "$TEST_ROOT/releases/release/bin/aliyun-signer"
...
deny runuser -u "$AUDIT_USER" -- test -r "$TEST_ROOT/releases/release/bin/private-runtime"
deny runuser -u "$GITHUB_USER" -- ls "$TEST_ROOT/releases/release/bin"
deny runuser -u "$ALIYUN_USER" -- ls "$TEST_ROOT/releases/release/bin"
deny runuser -u "$AUDIT_USER" -- ls "$TEST_ROOT/releases/release/bin"

echo 'provider and audit signer release ACLs: isolated execute-only traversal passed'
