...
console.log('=== secrets list ===');
{
  ...
  await handleSecrets(
    {},
    res,
    { method: 'GET', pathname: '/api/v1/secrets' },
    {
      send: (r, s, b) => {
        r.status = s;
        r.body = b;
...
console.log('=== services list ===');
{
  ...
  await handleServices(
    {},
    res,
    { method: 'GET', pathname: '/api/v1/services' },
    {
      send: (r, s, b) => {
        r.status = s;
        r.body = b;
...
console.log('=== clients admin only ===');
{
  ...
  await handleClients(
    {},
    res,
    { method: 'GET', pathname: '/api/v1/clients' },
    {
      send: () => {},
      jsonError: (r, s, m) => {
        r.status = s;
...
console.log('=== proxy missing service ===');
{
  ...
  await handleProxy(
    {},
    res,
    { method: 'GET', pathname: '/api/v1/proxy/nope/x' },
    {
      send: () => {},
      jsonError: (r, s, m) => {
        r.status = s;
...
console.log('=== proxy ok path ===');
{
  ...
  await handleProxy(
    {},
    res,
    { method: 'GET', pathname: '/api/v1/proxy/gh/repos' },
    {
      send: () => {},
      jsonError: (r, s, m) => {
        r.status = s;
