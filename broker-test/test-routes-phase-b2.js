...
console.log('=== handleHealth ===');
{
  ...
  assert(
    (await handleHealth({}, res, { method: 'POST', pathname: '/health' }, { send })) === false,
    'POST not handled',
  );
}
{
...
}
{
  ...
  const hidden = await handleHealth(
    {},
    res,
...
console.log('=== handleStatic ===');
{
  ...
  assert(
    handleStatic({}, res, { method: 'GET', pathname: '/nope' }, { dashboardDir: dir }) === false,
    'unknown path',
  );
