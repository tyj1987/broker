...
const githubCommits = registry.find('github', 'commits.list');
assert.equal(githubCommits.name, 'github.commits.list');
assert.equal(githubCommits.input_schema.properties.since.type, 'string');
assert.ok(githubCommits.audit_policy.redact.includes('signature'));
const githubIssues = registry.find('github', 'issues.list');
assert.equal(githubIssues.name, 'github.issues.list');
assert.equal(githubIssues.input_schema.properties.labels.maxItems, 10);
assert.ok(githubIssues.audit_policy.redact.includes('body'));
