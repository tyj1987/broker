#!/usr/bin/env node
// Build-time only: a closed dependency set for the independent exporter UID.
...
export const EXPORTER_FILES = Object.freeze([
  'bin/audit-exporter-service-check.js',
  ...['audit-anchor-exporter-runtime', 'audit-anchor-exporter', 'audit-recovery-chain',
    'audit-anchor', 'audit-hash-chain', 'redact', 'local-audit-anchor-signer-client',
    'local-audit-anchor-store-client'].map(name => `lib/${name}.js`),
]);
const digest = data => createHash('sha256').update(data).digest('hex');
const error = () => { throw new Error('Exporter package is invalid'); };
...
export function packageExporter(root, { verify = false } = {}) {
  const target = join(root, 'exporter-runtime');
  const expectedNames = ['package.json', ...EXPORTER_FILES, 'node_modules/yaml/package.json',
  ...
  const stage = mkdtempSync(join(root, '.exporter-package-'));
  const files = {};
  ...
  try {
    ...
    for (const name of EXPORTER_FILES) copy(join(root, name), name);
  ...
  return packageExporter(root, { verify: true });
}
...
if (process.argv[1] && fileURLToPath(import.meta.url) === resolve(process.argv[1])) {
  try {
    ...
    const result = packageExporter(root, { verify: args[0] === '--verify' });
    console.log(JSON.stringify({ status: 'exporter_package_verified', files: result.files }));
  } catch { console.error('exporter_package_invalid'); process.exitCode = 1; }
}
