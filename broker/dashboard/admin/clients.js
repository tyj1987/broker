...
// Lists clients and supports create/edit/delete/enroll/rotate/revoke.
// Enrollment and rotation return a one-time in-memory bundle; secret material
// is never logged, persisted to localStorage, or fetched from a repeatable URL.
...
(function () {
  ...
  let privateKeyRetentionEnabled = false;
  let editingName = null;
  let enrollSecretTimer = null;
  ...
  async function loadClients() {
    ...
    try {
      ...
      privateKeyRetentionEnabled = r.private_key_retention_enabled === true;
  ...
  async function updateClient(name, payload) {
    return api(`/api/v1/admin/clients/${encodeURIComponent(name)}`, {
      method: 'PUT',
      body: JSON.stringify(payload),
    });
  }
  async function deleteClient(name, verify) {
    return api(`/api/v1/admin/clients/${encodeURIComponent(name)}`, {
      method: 'DELETE',
      body: JSON.stringify({ verify }),
    });
  }
  async function enrollClient(name, verify) {
    return api(`/api/v1/admin/clients/${encodeURIComponent(name)}/enrollment`, {
      method: 'POST',
      body: JSON.stringify({ verify }),
    });
  }
  async function rotateClient(name, verify) {
    return api(`/api/v1/admin/clients/${encodeURIComponent(name)}/rotate`, {
      method: 'POST',
      body: JSON.stringify({ verify }),
    });
  }
  async function revokeClient(name, verify) {
    return api(`/api/v1/admin/clients/${encodeURIComponent(name)}/revoke`, {
      method: 'POST',
      body: JSON.stringify({ verify }),
    });
  }

  function promptVerification(action) {
    const value = window.prompt(
      `${action}\n请输入当前 TOTP、恢复码或管理员密码：\nEnter current TOTP, recovery code, or admin password:`,
      '',
    );
  ...
  function renderTable() {
    ...
    if (clients.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="7" class="muted">（还没有任何客户端）— 点右上角"+ 新增"添加第一个。</td></tr>';
      return;
    }
    tbody.innerHTML = '';
    for (const c of clients) {
      ...
      const certStatus = c.cert_present_on_disk
        ...
        : c.cert_fingerprint_sha256
          ? '<span class="badge-warn">⚠ 配置有 fp, 文件缺失</span>'
          : '<span class="muted-cell">未签发</span>';
      const keyStatus = c.cert_key_present_on_disk
        ? `<br><span class="badge-warn">⚠ ${
            privateKeyRetentionEnabled ? '兼容模式保留私钥' : '发现遗留私钥文件'
          }</span>`
        : '';
  ...
  function showTableError(msg) {
    const tbody = $('#admin-clients-table tbody');
    if (tbody)
      tbody.innerHTML = `<tr><td colspan="7" class="status-error">加载失败: ${esc(msg)}</td></tr>`;
  }
      ...
      $('#cl-allowed-proxy').value = (r.allowed_proxy || [])
        .map((p) =>
          typeof p === 'string'
            ? p
            : p.service
              ? p.service + (p.paths ? '  paths=' + JSON.stringify(p.paths) : '')
              : JSON.stringify(p),
        )
        .join('\n');
      $('#cl-error').hidden = true;
  ...
  async function onSave() {
    ...
    const verify = promptVerification(editingName ? '保存客户端配置' : '创建客户端');
    if (!verify) return showClientError('操作已取消：需要二次验证 / Verification required');
    cfg.verify = verify;
    try {
  ...
  function parseProxyLines(text) {
    return (text || '')
      ...
      .map((line) => {
        ...
        let paths;
        try {
          paths = m[2] ? JSON.parse(m[2]) : undefined;
        } catch {
          paths = undefined;
        }
  ...
  async function confirmDelete(name) {
    if (
      ...
      return;
    const verify = promptVerification(`删除客户端 ${name}`);
    if (!verify) return;
    try {
      await deleteClient(name, verify);
      await loadClients();
    } catch (e) {
  ...
  async function confirmRevoke(name) {
    if (
      !confirm(
        `确定撤销 ${name} 的证书？\n将立即从 broker 配置中移除指纹，设备下次连接会被拒。\nRevoke cert for ${name}?`,
      )
    )
      return;
    const verify = promptVerification(`撤销 ${name} 的证书`);
    if (!verify) return;
    try {
      await revokeClient(name, verify);
      await loadClients();
    } catch (e) {
  ...
  async function runEnroll(name) {
    const verify = promptVerification(`签发/补发 ${name} 的客户端证书`);
    if (!verify) return;
    ...
    try {
      const r = await enrollClient(name, verify);
      showEnrollResult(r);
      await loadClients();
    } catch (e) {
  ...
  async function runRotate(name) {
    if (
      !confirm(
        `轮换 ${name} 的证书？\n旧指纹和旧证书会话将立即失效；新私钥仅在结果窗口中提供一次。\nRotate ${name}?`,
      )
    )
      return;
    const verify = promptVerification(`轮换 ${name} 的客户端证书`);
    if (!verify) return;
    ...
    try {
      const r = await rotateClient(name, verify);
      showEnrollResult(r);
      await loadClients();
    } catch (e) {
      showEnrollResult({ ok: false, error: e.message });
  ...
  function showEnrollResult(r) {
    ...
    if (!modal || !out) {
      alert(r.ok ? 'OK' : '失败: ' + r.error);
      return;
    }
    if (!r.ok) {
      out.innerHTML = `<div class="status-error">${esc(r.error || '未知错误')}</div>`;
    } else {
      // Show the cert/key and one-time in-memory ZIP. Never log or persist
      // any of these values to localStorage or a repeatable server URL.
      const bundleButton = r.bundle_base64
        ? '<button class="btn btn-primary" id="btn-download-one-time-bundle">下载一次性 zip 安装包</button>'
        : '';
      out.innerHTML = `
      ...
      setTimeout(() => {
        ...
        const a = $('#btn-copy-cert');
        if (a) a.addEventListener('click', () => copy('enroll-cert'));
        const b = $('#btn-copy-key');
        if (b) b.addEventListener('click', () => copy('enroll-key'));
        const download = $('#btn-download-one-time-bundle');
        if (download) {
          download.addEventListener('click', () => {
            downloadOneTimeBundle(r.bundle_base64, r.name);
            r.bundle_base64 = '';
            download.disabled = true;
            download.textContent = '已下载（请妥善保存）';
          });
        }
      }, 0);
    }
    if (enrollSecretTimer) clearTimeout(enrollSecretTimer);
    enrollSecretTimer = setTimeout(closeEnrollResult, 5 * 60_000);
    modal.hidden = false;
  }

  function closeEnrollResult() {
    if (enrollSecretTimer) clearTimeout(enrollSecretTimer);
    enrollSecretTimer = null;
  ...
  function downloadOneTimeBundle(base64, name) {
    if (!base64) return;
    try {
      ...
      alert('一次性安装包解码失败，请复制 PEM 内容并手动保存。');
    }
  }
  ...
  function wireBundleModal() {
    ...
    if (modal)
      modal.addEventListener('click', (e) => {
        if (e.target === modal) closeEnrollResult();
      });
    document.addEventListener('keydown', (e) => {
