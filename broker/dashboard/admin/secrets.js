...
(function () {
  ...
  function buildFieldInput(f, currentValue, opts) {
    ...
    if (f.kind === 'select') {
      ...
      inputHtml = `<textarea id="${id}" name="${id}" rows="${isKeyLike ? 6 : 3}"${isRequired ? ' required' : ''} placeholder="${escapeHtml(f.placeholder || '')}">${escapeHtml(displayVal || '')}</textarea>`;
    } else if (f.kind === 'checkbox') {
      ...
      inputHtml = `<input id="${id}" name="${id}" type="${type}" value="${escapeHtml(val || '')}"${isRequired ? ' required' : ''} placeholder="${escapeHtml(f.placeholder || '')}">`;
    }
    if (f.kind === 'checkbox') {
      wrap.innerHTML = helpText ? `${inputHtml}<div class="hint" style="margin-top:4px">${escapeHtml(helpText)}</div>` : inputHtml;
    } else {
      wrap.innerHTML = `<div class="sf-field-label-text">${escapeHtml(labelText)}</div>${inputHtml}${helpText ? `<div class="hint">${escapeHtml(helpText)}</div>` : ''}`;
    }
    return wrap;
  }
