...
export const TYPE_SCHEMAS = {
  ...
  github_pat: {
    ...
    description:
      'https://github.com/settings/tokens — classic (ghp_) 或 fine-grained (github_pat_)',
  ...
  gitee_pat: {
    ...
    fields: [{ name: 'token', label: 'Token', kind: 'textarea', required: true, sensitive: true }],
  },
    ...
    fields: [
      {
        ...
        label: 'Service Account JSON (完整粘贴)',
        kind: 'textarea',
        required: true,
        sensitive: true,
  ...
  ssh_connection: {
    ...
    fields: [
      ...
      { name: 'port', label: '端口', kind: 'number', default: 22, help: '默认 22，留空使用默认' },
      { name: 'username', label: '用户名', kind: 'text', required: true, placeholder: 'root' },
      {
        name: 'known_hosts',
        label: '受信任主机密钥',
        kind: 'textarea',
  ...
  redis_url: {
    ...
    fields: [
      ...
      { name: 'db', label: 'DB 编号', kind: 'number', default: 0, help: '默认 0，范围 0-15' },
  ...
  stripe_secret_key: {
    ...
    fields: [
      ...
      {
        name: 'webhook_secret',
        label: 'Webhook Signing Secret (可选)',
        kind: 'password',
        sensitive: true,
        placeholder: 'whsec_...',
      },
  ...
  wechat_pay_key: {
    ...
    fields: [
      ...
      {
        name: 'private_key',
        label: '商户私钥 (apiclient_key.pem 内容)',
        kind: 'textarea',
        required: true,
        sensitive: true,
        file_upload: true,
  ...
  random_string: {
    ...
    fields: [{ name: 'value', label: '字符串', kind: 'textarea', required: true, sensitive: true }],
  },
};
...
const V4_TYPE_SCHEMAS = {
  ...
  dingtalk_app: {
    ...
    fields: [
      ...
      { name: 'agent_id', label: 'AgentId', kind: 'text', required: true, help: '机器人或应用 ID' },
    ],
  },
  // ---- 支付 ----
  wechat_miniprogram: {
