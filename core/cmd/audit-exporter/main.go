// The exporter coordinates typed signing/publication without receiving keys.
// A native supervisor preserves the independent workload and release identity.
package main
...
const exporterConfig = "/etc/secret-broker/audit/exporter.json"
...
func parseReport(raw []byte) (report, error) {
	...
	if len(raw) == 0 || len(raw) > maxOutput || json.Unmarshal(raw, &result) != nil ||
		result.Status != "anchor_verified" || result.Sequence < 1 ||
		result.Sequence > 9007199254740991 || result.IntervalMs < 60000 || result.IntervalMs > 3600000 {
...
func run(ctx context.Context, args []string, d dependencies) (int, string) {
	if len(args) != 2 || args[0] != "--config" || args[1] != exporterConfig {
...
func defaultDependencies() dependencies {
	...
	return dependencies{
		...
		wait:   waitDuration, output: os.Stdout,
	}
}
...
func runMain(args []string, stderr io.Writer) int {
	...
	if reason != "" {
		_, _ = fmt.Fprintf(stderr, "audit_exporter_failed=%s\n", reason)
	}
	return code
}
