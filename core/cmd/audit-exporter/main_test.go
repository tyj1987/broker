...
const success = `{"status":"anchor_verified","sequence":3,"interval_ms":60000}`
...
func validDependencies(ctx context.Context, cancel context.CancelFunc) dependencies {
	return dependencies{prepare: func() (string, error) { return "fixed-script", nil },
		...
		notify: func(string) error { return nil }, wait: func(context.Context, time.Duration) error { cancel(); return ctx.Err() }, output: io.Discard}
}
func TestRunSuccessAndReread(t *testing.T) {
	...
	if code != 0 || reason != "" || count != 3 || ready != 1 || strings.Count(output.String(), "anchor_verified") != 3 {
...
func TestRunFailures(t *testing.T) {
	cases := []struct {
		...
		{"wait", "interval_failed", 70, func(d *dependencies) {
			d.wait = func(context.Context, time.Duration) error { return errors.New("private detail") }
		}},
	}
	...
	t.Run("failure after ready exits", func(t *testing.T) {
		...
		if code, _ := run(ctx, []string{"--config", exporterConfig}, d); code != 69 || calls != 2 {
...
func TestRunArgumentsAndDependencies(t *testing.T) {
	...
	defer cancel()
	d := validDependencies(ctx, cancel)
	for _, args := range [][]string{nil, {"--config"}, {"--config", "/tmp/private"}, {"--config", exporterConfig, "extra"}, {"--other", exporterConfig}} {
	...
	for i := 0; i < 6; i++ {
		...
		if code, _ := run(ctx, []string{"--config", exporterConfig}, copy); code != 70 {
...
func TestCancellationAndNoPrematureReadiness(t *testing.T) {
	...
	if code, _ := run(ctx, []string{"--config", exporterConfig}, d); code != 0 || called {
		t.Fatal("cancelled check became ready")
	}
	if code, _ := run(ctx, []string{"--config", exporterConfig}, d); code != 0 {
		t.Fatal("pre-abort failure")
	}
	if waitDuration(ctx, time.Minute) == nil {
...
func TestReportStrictness(t *testing.T) {
	...
	cases := []string{"", "null", "{}", strings.Repeat("x", 1025), success + success, strings.Replace(success, `"sequence":3`, `"sequence":3,"sequence":3`, 1), strings.Replace(success, `"anchor_verified"`, `"verified"`, 1), strings.Replace(success, `"interval_ms":60000`, `"interval_ms":59999`, 1), strings.Replace(success, `"interval_ms":60000`, `"interval_ms":3600001`, 1), strings.ReplaceAll(success, ":3", ":0"), strings.Replace(success, `"sequence":3`, `"sequence":9007199254740992`, 1), strings.Replace(success, `"sequence":3`, `"sequence":3.5`, 1), strings.Replace(success, `"sequence":3`, `"sequence":3,"extra":true`, 1)}
...
func TestNodeCommandHasNoInheritedAuthority(t *testing.T) {
	...
	cmd := nodeCommand(context.Background(), "/fixed/exporter-runtime/bin/check.js")
	if cmd.Path != nodeRuntime || cmd.Dir != "/fixed/exporter-runtime" {
		t.Fatal("wrong path")
	}
	b, _ := json.Marshal(cmd.Args)
	if !strings.Contains(string(b), "--jitless") || !strings.Contains(string(b), "--no-addons") || !strings.Contains(string(b), exporterConfig) {
...
func TestChildHelper(t *testing.T) {
	switch os.Getenv("EXPORTER_TEST_CHILD") {
	case "ok":
...
func TestRealChildBoundaries(t *testing.T) {
	for _, mode := range []string{"ok", "fail", "large", "hang"} {
		t.Run(mode, func(t *testing.T) {
			...
			cmd.Env = append(os.Environ(), "EXPORTER_TEST_CHILD="+mode, "GORACE=atexit_sleep_ms=0")
	...
	if _, err := execute(ctx, exec.Command("/nonexistent-exporter-test")); err == nil {
...
func TestDefaultWiringAndEntryFailure(t *testing.T) {
	...
	if d.notify("READY=1") == nil || d.wait(ctx, time.Minute) == nil {
		t.Fatal("default boundary")
	}
	var stderr bytes.Buffer
	if code := runMain([]string{"--wrong", "synthetic-private-detail"}, &stderr); code != 64 || stderr.String() != "audit_exporter_failed=usage_invalid\n" {
