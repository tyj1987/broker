...
func look(name string) (*user.User, error) {
	if name == "broker-audit-recovery" {
		return &user.User{Uid: "1234", Gid: "1234"}, nil
	}
	return &user.User{Uid: "2000", Gid: "2000"}, nil
}
func group(string) (*user.Group, error) { return &user.Group{Gid: "1202"}, nil }
func TestIdentityMustBeSeparateAndExplicit(t *testing.T) {
	if validateIdentity(1234, 1234, []int{1234, 1202}, look, group) != nil {
		t.Fatal("valid identity")
	}
	for _, c := range []struct {
		u, g   int
		groups []int
	}{{0, 1234, []int{1202}}, {1234, 0, []int{1202}}, {1, 1234, []int{1202}}, {1234, 1, []int{1202}}, {1234, 1234, nil}, {1234, 1234, []int{1202, 0}}, {1234, 1234, []int{1202, 9999}}} {
	...
	for _, bad := range []func(string) (*user.User, error){nil, func(string) (*user.User, error) { return nil, errUnavailable }, func(string) (*user.User, error) { return nil, nil }, func(string) (*user.User, error) { return &user.User{Uid: "1234", Gid: "1234"}, nil }} {
		if validateIdentity(1234, 1234, []int{1202}, bad, group) == nil {
	...
	for _, bad := range []func(string) (*user.Group, error){nil, func(string) (*user.Group, error) { return nil, errUnavailable }, func(string) (*user.Group, error) { return nil, nil }, func(string) (*user.Group, error) { return &user.Group{Gid: "bad"}, nil }, func(string) (*user.Group, error) { return &user.Group{Gid: "0"}, nil }, func(string) (*user.Group, error) { return &user.Group{Gid: "1234"}, nil }} {
		if validateIdentity(1234, 1234, []int{1202}, look, bad) == nil {
			t.Fatal("group accepted")
		}
	}
	if _, err := prepareRecovery(); err == nil {
...
func TestImmutablePathsRejectLinksAndWritableAncestors(t *testing.T) {
	...
	if !releaseExecutable.MatchString("/opt/secret-broker/releases/" + strings.Repeat("a", 40) + "/bin/secret-broker-audit-recovery") {
		t.Fatal("valid release rejected")
	}
	for _, p := range []string{"/tmp/recovery", "/opt/secret-broker/broker/bin/secret-broker-audit-recovery", "/opt/secret-broker/releases/" + strings.Repeat("A", 40) + "/bin/secret-broker-audit-recovery"} {
...
func TestPreparationBindsNativeAndNodeCodeToOneRelease(t *testing.T) {
	own := "/opt/secret-broker/releases/" + strings.Repeat("a", 40) + "/bin/secret-broker-audit-recovery"
	script := "/opt/secret-broker/releases/" + strings.Repeat("a", 40) + "/recovery-runtime/bin/audit-recovery-service-check.js"
	...
	got, err := prepareWith(1234, 1234, []int{1202}, look, group, exe, stat)
	...
	for _, bad := range []func() (string, error){nil, func() (string, error) { return "", errUnavailable }, func() (string, error) { return "/tmp/other", nil }} {
		if _, err = prepareWith(1234, 1234, []int{1202}, look, group, bad, stat); err == nil {
			t.Fatal("invalid exe")
		}
	}
	for _, target := range []string{own, nodeRuntime, script} {
		...
		if _, err = prepareWith(1234, 1234, []int{1202}, look, group, exe, badStat); err == nil {
