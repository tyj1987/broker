...
7. Create the four fixed audit users and groups, then install the reviewed audit signer, exporter, immutable-store, and recovery-authority units listed above. These checked-in units are deployment contracts, not runnable services until their matching binaries and configuration validators are packaged. Do not substitute placeholder commands or a shared `nobody` identity. The preflight intentionally remains not ready until all four units are active under their exact, mutually distinct identities; a local JSONL chain alone is not production audit evidence.
