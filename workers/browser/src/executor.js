...
const FORBIDDEN_INPUT_KEYS =
  /^(?:url|uri|selector|script|javascript|headers?|cookies?|storage_state|storageState|devtools|cdp)$/i;
const SENSITIVE_OUTPUT_KEYS =
  /(?:secret|token|password|authorization|cookie|session|credential|private.?key|otp|code)/i;
...
function inspectTypedValue(value, depth = 0) {
  if (depth > 8)
    throw new BrowserWorkerError('invalid_request', 'typed_parameters is too deeply nested');
  if (value === null || ['string', 'number', 'boolean'].includes(typeof value)) return;
  if (Array.isArray(value)) {
    if (value.length > 100)
      throw new BrowserWorkerError('invalid_request', 'typed_parameters array is too large');
  ...
  if (typeof value !== 'object')
    throw new BrowserWorkerError(
      'invalid_request',
      'typed_parameters contains an unsupported value',
    );
  for (const [key, item] of Object.entries(value)) {
    if (FORBIDDEN_INPUT_KEYS.test(key)) {
      throw new BrowserWorkerError(
        'forbidden_parameter',
        `typed parameter is controlled by the adapter: ${key}`,
      );
    }
...
function normalizedDescriptor(adapter) {
  ...
  if (typeof adapter.execute !== 'function')
    throw new BrowserWorkerError('invalid_adapter', 'adapter execute function is required');
  let start;
  try {
    start = new URL(adapter.startUrl);
  } catch {
    throw new BrowserWorkerError('invalid_adapter', 'adapter start URL is invalid');
  }
  if (start.protocol !== 'https:' || start.username || start.password || start.hash) {
    throw new BrowserWorkerError(
      'invalid_adapter',
      'adapter start URL must be a credential-free HTTPS URL',
    );
  }
  ...
  for (const origin of origins) {
    if (!origin.startsWith('https://'))
      throw new BrowserWorkerError('invalid_adapter', 'adapter origins must use HTTPS');
  }
...
function inspectResult(value, depth = 0) {
  if (depth > 8)
    throw new BrowserWorkerError('unsafe_result', 'adapter result is too deeply nested');
  if (value === null || ['string', 'number', 'boolean'].includes(typeof value)) return;
  if (Array.isArray(value)) {
  ...
  if (typeof value !== 'object')
    throw new BrowserWorkerError('unsafe_result', 'adapter result contains an unsupported value');
  for (const [key, item] of Object.entries(value)) {
    if (SENSITIVE_OUTPUT_KEYS.test(key))
      throw new BrowserWorkerError('unsafe_result', 'adapter result contains a sensitive field');
...
function safeResult(value) {
  ...
  try {
    ...
    throw new BrowserWorkerError('unsafe_result', 'adapter result is not serializable');
  }
  if (Buffer.byteLength(encoded) > MAX_RESULT_BYTES)
    throw new BrowserWorkerError('unsafe_result', 'adapter result is too large');
...
export class BrowserOperationExecutor {
  constructor({ browser, adapters, timeoutMs = 60_000 }) {
    if (!browser || typeof browser.newContext !== 'function')
      throw new BrowserWorkerError('invalid_runtime', 'browser is required');
    ...
    for (const adapter of adapters || []) {
      ...
      if (this.adapters.has(key))
        throw new BrowserWorkerError('invalid_adapter', 'duplicate adapter');
  ...
  async execute(request, runtime = {}) {
    ...
    for (const key of Object.keys(request)) {
      if (!REQUEST_KEYS.has(key))
        throw new BrowserWorkerError('forbidden_parameter', `request field is not allowed: ${key}`);
    }
    ...
    if (!ENVIRONMENTS.has(request?.environment))
      throw new BrowserWorkerError('invalid_request', 'environment is invalid');
    ...
    try {
      ...
      throw new BrowserWorkerError('invalid_request', 'typed_parameters is not serializable');
    }
    ...
    if (!adapter)
      throw new BrowserWorkerError('operation_denied', 'operation has no reviewed browser adapter');
